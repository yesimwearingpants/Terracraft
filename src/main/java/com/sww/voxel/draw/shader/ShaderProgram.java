...
public abstract class ShaderProgram {
	...
	protected void loadVector(int location, Vector3fExt vector) {
		GL20.glUniform3f(location, vector.getX(), vector.getY(), vector.getZ());
	}

	protected void loadVector(int location, Vector4fExt vector) {
	...
	private static int loadShader(String file, int type) {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(String.format("assets/shaders/%s", file)))) {
			String line;
		...
		if (shaderID == 0)
			throw new OpenGLException("OpenGL Context Doesn't Support Shaders");
